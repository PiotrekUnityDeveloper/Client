...
public class GuiThemes {
    private static final File FOLDER = new File(MatHax.VERSION_FOLDER, "GUI");
    ...
    public static void add(GuiTheme theme) {
        for (Iterator<GuiTheme> it = themes.iterator(); it.hasNext();) {
            if (it.next().name.equals(theme.name)) {
                ...
                MatHax.LOG.error(MatHax.LOG_PREFIX + "Theme with the name '{}' has already been added.", theme.name);
                break;
            }
        }
